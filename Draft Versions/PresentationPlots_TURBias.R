...
#--------------------- Plot to illustrate Bayesian inference -----------------------------
...
dp <- p_vals[2] - p_vals[1]  # step size

# Compute distributions
prior <- dbeta(p_vals, alpha, beta)
likelihood_raw <- dbinom(y, size = N, prob = p_vals)
likelihood <- likelihood_raw / sum(likelihood_raw * dp)  # normalize
posterior <- dbeta(p_vals, posterior_alpha, posterior_beta)

# Plot
plot(p_vals, prior, type = "l", col = "blue", lwd = 2,
     ylim = c(0, max(c(prior, likelihood, posterior))),
     ylab = "Density", xlab = expression(p),
     main = "")
lines(p_vals, likelihood, col = "darkgreen", lwd = 2)
lines(p_vals, posterior, col = "tomato", lwd = 2)

legend("topright", legend = c("Prior", "Likelihood (normalized)", "Posterior"),
       col = c("blue", "darkgreen", "tomato"), lwd = 2, y.intersp = 1.5)
...
plot(field_data$longitude, field_data$latitude)
...
plot(dat$build_counts, dat$build_dam_counts/dat$build_counts)
#dat %<>% filter(pga_mean > -2)

plot(dat$pgv_mean, dat$build_dam_counts/dat$build_counts)
...
xx = dat %>% filter(pgv_mean > 50) %>% group_by(build_counts = round(build_counts/2)*2) %>% summarize(propDam = mean(build_dam_counts!=0))
# 
# 
plot(log(xx$build_counts), xx$propDam, xlab='Building Density', ylab='Proportion with at least one damaged building')
...
lines(log(df_testsim$build_count), df_testsim$prop_dam)

#0.04 - 0.01
xgr = seq(0, 1000, 1)
yax = 0.1* sqrt(xgr)
ablines = data.frame(xgr = xgr, yax = yax)
ablines$yax[ablines$yax>1] = 1
ggplot(xx, aes(x=build_counts, y=propDam)) + geom_point() +
  scale_x_continuous(
    trans = scales::pseudo_log_trans(sigma = 10, base = 10),
    breaks = c(0, 20, 50, 100, 200,500, 1000),
    labels = scales::comma_format(),
    minor_breaks = NULL
  ) + theme_minimal() + xlab('Building Density') + ylab('Proportion with at least one damaged building') +
  geom_line(data=df_testsim, aes(x=build_count, y=prop_dam), col='red') + 
  geom_line(data=ablines, aes(x=xgr, y=yax), col='blue')

yy = dat %>% filter(pgv_mean > 50 & build_counts > 100)
sum(yy$build_dam_counts)/sum(yy$build_counts)

ministrys_df_full$build_dens = extract(rast(build_counts), vect(ministrys_df_full, geom = c("longitude", "latitude")))$layer
...
#---------------------------------------------------------------------------------------------
# ------------------------- MODEL FIT 1: MULTINOMIAL NO ZONING -------------------------------
#---------------------------------------------------------------------------------------------

stan_model_compiled <- stan_model("/home/manderso/Documents/GitHub/TUR2023_02_06/MultinomialNoZoning.stan")
...
stan_data <- list(
  ...
  lambda = 0.7,
  concentration_zone = 50
  #p_ministrys_unobserved_param= 1900
)

control = list(adapt_delta = 0.99, max_treedepth = 15)

fit_MultinomialNoZoning <- sampling(
...
# ------------ Plot prior vs posterior probability of different building types ----------------

posterior_samples <- as_draws_df(fit_MultinomialNoZoning)
...
n_samples = 10000
prior_samps <- rdirichlet(n_samples, buildingtypes_priorprob)
...
city_indices <- c("Kahramanmaras" = 3)
...
posterior_list <- lapply(names(city_indices), function(city_name) {
  ...
  lapply(seq_along(structure_types), function(i) {
    col_name <- grep(paste0("^buildingTypeProbs\\[", city_index, ",", i, "\\]"), names(posterior_samples), value = TRUE)
    tibble(
...
plot_list <- lapply(unique(plot_df$structure_type), function(st) {
  ggplot(filter(plot_df, structure_type == st), aes(x = value, fill = source)) +
    geom_histogram(aes(y = ..density..), position = "identity", alpha = 0.65, bins = 40) +
    labs(
      x = "Probability of building type",
      y = "Density",
      fill = "Source",
      title = st
    ) + 
...
#----------------------------------------------------------------------------------------------
#------------------ Compare multinomial vs multinomial dirichlet ------------------------------
#----------------------------------------------------------------------------------------------
library(MGLM)

samp = rdirmn(1, 100, rep(0.2,5))
hist(samp)

par(mfrow=c(2,2), mar = c(3, 3, 2, 1))
for (i in 1:4){
  samp = rmultinom(1, 100, rep(0.2, 5))
  names(samp) = c('A', 'B', 'C', 'D', 'E')
  
  samp_vec <- as.vector(samp)
  names(samp_vec) <- c('A', 'B', 'C', 'D', 'E')
...
par(mfrow=c(2,2), mar = c(3, 3, 2, 1))
for (i in 1:4){
  samp = rdirmn(1, 100, rep(0.2, 5)*7.5)
  names(samp) = c('A', 'B', 'C', 'D', 'E')
...
stan_model_compiled <- stan_model("/home/manderso/Documents/GitHub/TUR2023_02_06/FragModelOG.stan")
...
#stan_model_compiled <- stan_model("/home/manderso/Documents/GitHub/TUR2023_02_06/CombinedModel.stan")
stan_model_compiled <- stan_model("/home/manderso/Documents/GitHub/TUR2023_02_06/MinistrysFragCurves.stan")
...
stan_data <- list(
  ...
  build_dens = ministrys_df$build_dens,
  p_ministrys_unobserved_param = 0.05
)
...
plot(posterior_samples$mu, posterior_samples$sigma)

mu_post <- pull(posterior_samples[, "mu"])
sigma_post <- pull(posterior_samples[, "sigma"])
...
#stan_model_compiled <- stan_model("/home/manderso/Documents/GitHub/TUR2023_02_06/CombinedModel.stan")
stan_model_compiled <- stan_model("/home/manderso/Documents/GitHub/TUR2023_02_06/CombinedModel6.stan")
...
n_samples = 10000
zone_means_prior = rdirichlet(n_samples, buildingtypes_priorprob)
city_means_prior = rdirichlet(n_samples, buildingtypes_priorprob)
lambda <- stan_data$lambda  # or sample from its prior
concentration_zone <- rgamma(n_samples, 50, 1)  # adjust to match your prior, or sample from gamma(2, 0.1)
...
for (i in 1:n_samples) {
  ...
  alpha <- concentration_zone[i] * mixture_mean
...
posterior_list <- lapply(names(zone_indices), function(zone_name) {
  ...
  lapply(seq_along(structure_types), function(i) {
    col_name <- grep(paste0("^buildingTypeProbs\\[1,", zone_index, ",", i, "\\]"), names(posterior_samples), value = TRUE)
    tibble(
      structure_type = structure_types[i],
      value = posterior_samples[[col_name]],
