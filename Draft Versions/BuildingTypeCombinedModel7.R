...
plot(fit, pars=)
# n_samples = 100
# zone_means = rdirichlet(n_samples, buildingtypes_priorprob)
# city_means = rdirichlet(n_samples, buildingtypes_priorprob)
# overall_means = array(0, dim=c(n_samples, 11))
#   
# for (i in 1:n_samples){
#   alpha = 2 * (
#    0.8 * zone_means[i,] + (0.2) * city_means[i, ]
#   )
#   overall_means[i, ] = rdirichlet(1, alpha)
# }
# hist(overall_means[,2])

traceplot(fit, pars='buildingTypeProbs[3,2,3]')


pairs(fit, pars=c('obs_prob[1,11]', 'zone_prob[1,11]'))
...
posterior_list <- lapply(names(zone_indices), function(zone_name) {
  ...
  lapply(seq_along(structure_types), function(i) {
    ...
    tibble(
      structure_type = structure_types[i],
      value = posterior_samples[[col_name]],
      source = paste("Posterior -", zone_name)
    )
  }) %>% bind_rows()
}) %>% bind_rows()
...
plot_list <- lapply(unique(plot_df$structure_type), function(st) {
  ggplot(filter(plot_df, structure_type == st), aes(x = value, fill = source)) +
    ...
    scale_fill_manual(values = c(
      "Prior" = "skyblue",
      "Posterior - Zone 1" = "tomato",
      "Posterior - Zone 2" = "darkgreen"
    )) +
    theme_minimal() +
    theme(
      plot.title = element_text(hjust = 0.5, size = 10),
