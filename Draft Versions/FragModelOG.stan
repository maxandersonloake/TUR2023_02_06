...
model {
  ...
  for (n in 1:N_buildings_ministrys){
    ...
    for (z in 1:N_zones){
      vector[N_buildingTypes] log_lik_buildingtypes;

      for (k in 1:N_buildingTypes){
        real loc = mu[k];
        real p_dam = lognormal_cdf(updated_PGV_ministrys[n], loc, sigma[k]);

        // Log-likelihood for this building type
        log_lik_buildingtypes[k] = log(buildingTypeProbs[cities_ministrys[n], z][k])
                                 + bernoulli_lpmf(damage_flag_ministrys[n] | p_dam);
      }

      // Marginalize over building types for this zone
     log_lik_zone[z] = log(zone_probs[z]) + log_sum_exp(log_lik_buildingtypes);
    }

    // Marginalize over zones
