...
data {
  ...
  real<lower=0> dist_nearest_dam[N_buildings_ministrys];
...
transformed parameters {
  ...
  for (n in 1:N_buildings_ministrys){
    updated_PGV_ministrys[n] = exp(PGV_errors_ministrys[n] + log(PGV_ministrys[n]));
  }
  
  for (n in 1:N_buildings_ministrys){
    p_ministrys_unobserved[n] = 1- exp(-p_ministrys_unobserved_param * dist_nearest_dam[n]);
  }
  ...
  for (n in 1:N_cities) {
    ...
    vector[N_buildingTypes] temp = to_vector(obs_prob[n]) .* to_vector(p_buildingtype[n]);
...
model {
  ...
  for (n in 1:N_cities){
     for (k in 1:(N_buildingTypes - 1)){
       ...
       obs_prob_global[k] ~ normal(1, 0.4);
     }
     obs_prob_free[n, N_buildingTypes-1] ~ normal(obs_prob_global[N_buildingTypes-1], 0.1);
     obs_prob_global[N_buildingTypes-1] ~ normal(0.1, 0.2);
    ...
    for (k in 1:N_buildingTypes){
      ...
      if (damage_flag_ministrys[n] == 1) {
        ...
        lps[k] += log_sum_exp(
          ...
          log_cdf + log(p_ministrys_unobserved[n])                 // damaged but unobserved
        );
      }
    }
